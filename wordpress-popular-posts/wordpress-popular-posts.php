		...
		function get_popular_posts($instance, $echo = true) {		
			...
			switch( $instance['range'] ) {
				...
				case 'daily':
					...
					$range = $table."cache.day >= '".gmdate("Y-m-d H:i:s")."' - INTERVAL 1 DAY";
					break;
				case 'weekly':
...
/* loop-wordpress-popular-posts add only below code to wpp original one. */
function get_popular_posts_ids($atts) {		
	global $wpdb;
